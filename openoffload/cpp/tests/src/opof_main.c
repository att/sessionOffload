...
void opof_client_test(char *address, unsigned short port);
void opof_server_test(char *address, unsigned short port);
/**
 * Print configuration (Debugging utility)
 */
void print_config(arg_config_t *config){
    printf("\n---- Open Offload Test Utility ----\n"); 
    printf("Version %s\n", OPOF_VERSION);
    printf("----------------------------------------\n");
}
/*
...
int main(int argc, char ** argv){
     ...
     * Loop over input
     */
	while (( c = getopt_long(argc,argv, "a:p:csvh",longopts,NULL))!=    -1){
        switch(c) {
            case 'c':
                client_enable = true;
                server_enable = false;
    ...
    */
    ...
    if (client_enable == true ) {
        opof_client_test(address, port);
    } else if (server_enable == true) {
        opof_server_test(address, port);
    } 
